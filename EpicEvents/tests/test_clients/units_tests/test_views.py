...
class ClientViewsetTests(ClientTest):
    ...
    def test_client_list(self, user_email, status_code):
        ...
        if user.is_superuser:
            ...
            self.clients_test = Client.objects.filter(sales_contact_id=user).order_by('id') | Client.objects.filter(
                contract__event__support_contact_id=user).order_by('id')
    ...
    @parametrize('user_email, status_code', [(ADMIN_USER['email'], 200), (EMPLOYEES[0]['email'], 200), (EMPLOYEES[1]['email'], 200)])
    ...
    @parametrize('user_email, status_code', [(ADMIN_USER['email'], 200), (EMPLOYEES[0]['email'], 200), (EMPLOYEES[1]['email'], 403)])
    ...
    @parametrize('user_email, status_code', [(ADMIN_USER['email'], 204), (EMPLOYEES[0]['email'], 403), (EMPLOYEES[1]['email'], 403)])
